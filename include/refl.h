...
public:
    ...
    template<class R, class... P> void  BindFunction(std::string name, R (*func)(P...), std::initializer_list<const char *> paramNames) { functions.push_back(BindWithSignature(move(name), func, Tag<R(P...)>())); int i=0; for(auto pn : paramNames) { functions.back().SetParamName(i++, pn); } }
